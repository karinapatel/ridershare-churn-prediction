...
def my_model(model_name):
    ...
    #training stats
    print("TRAIN")
    print("ROC_AUC score: ", cross_val_score(model_name,df_train,y_train,cv=5,scoring='roc_auc').mean())
    print("Accuracy score: ", cross_val_score(model_name),df_train,y_train,cv=5).mean())
    print("f1 score: ", cross_val_score(model_name,df_train,y_train,cv=5,scoring='f1').mean())
